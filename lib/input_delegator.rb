class InputDelegator
  ...
  def is_valid_input(input, game)
    ...
    options.call(game, input).include?(input)
  end

  def new_options(_game, _input)
    ['ready']
  end

  def new_ready(game, _input)
  ...
  def started_options(game, _input)
    options = []
  ...
  def end_options(_game, _input)
    %w[new quit]
  end

  def end_close(game, _input)
    game.set_state(GAME_STATE_CLOSED)
  end

  def end_new(game, _input)
    game.reset
  end
